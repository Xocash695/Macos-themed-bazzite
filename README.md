...
> **Disclaimer:** This project is in no way affiliated with, endorsed by, or associated with Apple Inc. macOS is a trademark of Apple Inc. This repository does not host any Apple assets or artifacts — it simply automates the download, installation, and configuration of third-party open-source themes and tools inspired by macOS. The pre-built OCI image published to GHCR is the automated output of running `build.sh` via GitHub Actions, fetching everything at build time from the upstream sources listed in the Credits section. The only exception is a Konsave profile (`macOS-layout.knsv`) included in this repo, which contains a custom KDE panel and widget layout. The package is made available for existing users to receive updates via `bootc upgrade`. No ISO is distributed publicly for legal reasons.
